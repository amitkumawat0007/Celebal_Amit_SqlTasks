...
create TRIGGER surrender_BUHR ON AnaplanMay2018.dbo.[inprocess_simulation_AOP_CY_BUHR] 
FOR UPDATE
AS
...
select @PostTagCountry= i.country   from maaping_city_region i join [inprocess_simulation_AOP_CY_BUHR] on transfer_to_city=i.[city cluster] and [Anaplan ID]=@anaplanCode;
select @PostTagFacility=i.Facility   from maaping_city_region i join [inprocess_simulation_AOP_CY_BUHR] on transfer_to_city=i.[city cluster]  and [Anaplan ID]=@anaplanCode;
...
delete from [inprocess_simulation_AOP_CY_BUHR_update] where [Anaplan Code]=@anaplanCode
...
----------------------------------------------------------------------------------------------------------------------------------------
update [AnaplanMay2018].[dbo].[inprocess_simulation_AOP_CY_BUHR] SET 
...
update [AnaplanMay2018].[dbo].[inprocess_simulation_AOP_CY_BUHR] SET  [transfer_to_city] = 'Dubai', 
[transfer_to_region]='APAC', [transfer_to_subBand]='B3-1', [Cost]='', [user_name] = 'manil.kumar@tatacommunications.com', 
[is_transfer] = '1' WHERE [AID] = '6012537-P912099-477078' and [BU SSU] = 'Business Collaboration , Mobility & IoT Solutions'




	select top 10 * from [inprocess_simulation_AOP_CY_BUHR] WHERE [Report Type]='Headcount';
	select top 10 * from [inprocess_simulation_AOP_CY_BUHR_update] 
